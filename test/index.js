...
test('load external js', function (t) {
    ...
    pdf.create(enrichedHtml, {phantomArgs: ['--ignore-ssl-errors=true']})
        .toFile(filename, function (error, pdf) {
            t.error(error);
            t.assert(pdf.filename == filename, 'Returns the filename from the phantom script');
            t.assert(fs.existsSync(pdf.filename), 'Saves the pdf with a custom page size and footer');
...
test('load broken html', function (t) {
    t.plan(1);

    var enrichedHtml = fs.readFileSync(path.join(__dirname, 'broken.html'), 'utf8');
    var filename = path.join(__dirname, 'broken.pdf');
    pdf.create(enrichedHtml, {phantomArgs: ['--ignore-ssl-errors=true']})
        .toFile(filename, function (error, pdf) {
            t.assert(error != null, "error was returned for broken html file (as it should be)");
            // t.error(error);
            // t.assert(pdf.filename == filename, 'Returns the filename from the phantom script');
            // t.assert(fs.existsSync(pdf.filename), 'Saves the pdf with a custom page size and footer')
        });
...
test('load broken html and ignore errors', function (t) {
    t.plan(3);

    var enrichedHtml = fs.readFileSync(path.join(__dirname, 'broken.html'), 'utf8');
    var filename = path.join(__dirname, 'broken.pdf');
    pdf.create(enrichedHtml, {ignoreJavascriptErrors: true, phantomArgs: ['--ignore-ssl-errors=true']})
        .toFile(filename, function (error, pdf) {
            t.error(error);
            t.assert(pdf.filename == filename, 'Returns the filename from the phantom script');
            t.assert(fs.existsSync(pdf.filename), 'Saves the pdf without errors')
        });
});
